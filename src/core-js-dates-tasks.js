...
function getNextFriday(date) {
  ...
  nextDate.setUTCDate(nextDate.getUTCDate() + daysUntilFriday);
  return nextDate;
}
...
function getNextFridayThe13th(date) {
  ...
  let candidate = new Date(year, month, 13);

  while (candidate <= date || candidate.getDay() !== 5) {
    month += 1;
    if (month > 11) {
      month = 0;
      year += 1;
    }
    candidate = new Date(year, month, 13);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(formatDateString(currentDate));
